...
#
# %% [markdown]
# Our RBF kernel is parameterised by a lengthscale $\ell$ and variance parameter $\sigma^2$ and our likelihood controls the observation noise with $\alpha^2$. To optimise these, we begin by calling `initialise`.
# %%
...
# | `trainable`  | boolean dictionary that deterimes the training status of parameters (`True` for being trained and `False` otherwise)  |
# |  `constrainer` | transformations that map parameters from the unconstrained space back to their original space  |
# |  `unconstrainer` |  transformations that map parameters from their original space back to an unconstrained space for optimisation |
#
