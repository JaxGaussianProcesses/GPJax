...
try:
    ...
    Pos_Train =onp.genfromtxt('docs/examples/data/XY_train_vortex.csv', delimiter=',').T
    Pos_Test =onp.genfromtxt('docs/examples/data/XY_test_vortex.csv', delimiter=',').T
    Vel_Train =onp.genfromtxt('docs/examples/data/UV_train_vortex.csv', delimiter=',').T
...
ax.quiver(Pos_Train[0],Pos_Train[1], Vel_Train[0], Vel_Train[1], color = 'red', alpha = 0.7, label = '$D_T$')
...
PosTest3D = jnp.vstack((jnp.repeat(Pos_Test.reshape(2,289), repeats=2 , axis = 1), zLabelTest)).T
...
ResidualsVel = jnp.sqrt((Vel_Test[0].reshape(17,17)-Vel_Lat[0])**2+(Vel_Test[1].reshape(17,17)-Vel_Lat[1])**2)
X = Pos_Test[0]
Y = Pos_Test[1]
...
ax[1].set(xlim = [-1.3,1.3], ylim = [-1.3,1.3], aspect = 'equal', title = 'GP Latent Estimate')
...
#Residuals between ground truth and estimate
ResidualsHelm = jnp.sqrt((Vel_Test[0].reshape(17,17)-Helm_Lat[0])**2+(Vel_Test[1].reshape(17,17)-Helm_Lat[1])**2)
X = Pos_Test[0]
Y = Pos_Test[1]
...
ax[1].quiver(Pos_Train[0], Pos_Train[1], Vel_Train[0], Vel_Train[1], color = 'red')
ax[1].set(xlim = [-1.3,1.3], ylim = [-1.3,1.3], aspect = 'equal', title = 'GP Latent Estimate of $\mathbf{F}$')
#Residuals
im = ax[2].imshow(ResidualsHelm,  extent=[X.min(), X.max(), Y.min(), Y.max()], cmap = 'hot')
ax[2].set(xlim = [-1.3,1.3], ylim = [-1.3,1.3], aspect = 'equal', title = 'Residuals')
fig.colorbar(im, fraction=0.046, pad=0.04, orientation='vertical')
plt.show()
