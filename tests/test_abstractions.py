...
from gpjax import RBF, Prior, Gaussian, Dataset, initialise, transform
...
def test_fit(n):
    key = jr.PRNGKey(123)
    x = jnp.sort(
        jr.uniform(key=key, minval=-2.0, maxval=2.0, shape=(n, 1)), axis=0
    )
    ...
    optimised_params = fit(
        mll, params, opt_init, opt_update, get_params, n_iters=10
    )
