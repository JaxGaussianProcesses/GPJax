...
class VariationalPrecipGP(ApproxPrecipGP):
    ...
    list_of_kernels:List[gpx.kernels.AbstractKernel]
    ...
    def _predict(
        ...
        # Unpack test inputs
        t = test_inputs
        Ktt = self.eval_K_xt(t,t) # [L, N, N]
        Kzt = self.eval_K_xt(z,t) # [L, M, N]
    ...
    def eval_K_xt(self, x: Num[Array, "N d"], t: Num[Array, "M d"]) -> Num[Array, "L N M"]:
        return jnp.stack([k.cross_covariance(x,t) for k in self.list_of_kernels]) # [L, N, M]
...
# @dataclass
# class AdditiveVariationalPrecipGP(VariationalPrecipGP):
...
def init_kernels(problem_info:ProblemInfo, additive=True):
    ...
    if additive:
        ...
        return  gpx.kernels.RBF(lengthscale=jnp.array([1.1]*len(problem_info.names_short))).replace_bijector(lengthscale = lengthscale_bij)
